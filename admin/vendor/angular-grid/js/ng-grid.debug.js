...
var ngFooter = function ($scope, grid) {
    ...
    $scope.maxPages = function () {
        if($scope.pagingOptions.pageSize=="All"){
            return 1;
        }
        if ($scope.maxRows() === 0) {
            return 1;
    ...
    $scope.initialPage = function () {
        if (grid.data.length >= $scope.pagingOptions.pageSize){
            return ($scope.pagingOptions.currentPage - 1) * grid.data.length + 1;
        }
        else if($scope.pagingOptions.pageSize=="All"){
            return 1;
        }
        else
        {
            return ($scope.pagingOptions.currentPage - 1) * $scope.pagingOptions.pageSize + 1;
        }
    }

    $scope.lastPage = function(){
        if (grid.data.length >= $scope.pagingOptions.pageSize){
            return ($scope.pagingOptions.currentPage - 1) * grid.data.length + grid.data.length;
        }
        else if($scope.pagingOptions.pageSize=="All"){
        ...
        {
            return ($scope.pagingOptions.currentPage - 1) * $scope.pagingOptions.pageSize + grid.data.length;
        }
    }
};
...
angular.module('ngGrid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('footerTemplate.html',
    ...
    "\n" +
    "            <span class=\"ngLabel\">Showing {{initialPage()}} to {{lastPage()}} of {{maxRows()}} entries</span> | <div style=\"float:right; margin-right: 10px;\" class=\"ngRowCountPicker\">\r" +
    "\n" +
    "            <span style=\"float: left;\" class=\"ngLabel\">Show </span>\r" +
    "\n" +
    "            <select style=\"float: left;height: 22px; width: 55px;border-radius: 0px;margin: -1px 4px 0;\" ng-model=\"pagingOptions.pageSize\" >\r" +
    "\n" +
    "                <option ng-repeat=\"size in pagingOptions.pageSizes\">{{size}}</option>\r" +
    "\n" +
    "            </select>\r" +
    "\n" +
    "            <span style=\"float: left;\" class=\"ngLabel\"> records per page</span>\r" +
    "\n" +
    "        </div>\r" +
    "\n" +
    "        </div>\r" +
    ...
    "\n" +
    "            <button type=\"button\" class=\"ngPagerButton\" ng-click=\"pageBackward()\" ng-disabled=\"cantPageBackward()\" title=\"{{i18n.ngPagerPrevTitle}}\">{{i18n.ngPagerPrevTitle}}</button>\r" +
    "\n" +
    ...
    "\n" +
    "            <button type=\"button\" class=\"ngPagerButton\" ng-click=\"pageForward()\" ng-disabled=\"cantPageForward()\" title=\"{{i18n.ngPagerNextTitle}}\">{{i18n.ngPagerNextTitle}}</button>\r" +
    "\n" +
    "        </div>\r" +
    "\n" +
    "    </div>\r" +
