...
App.controller('DashboardController', function ($scope, $http, $cookies, $cookieStore, MY_CONSTANT, ngDialog) {
    ...
    $.get(MY_CONSTANT.url + '/api/admin/getCount/' +  $cookieStore.get('obj1') + '/' +  $cookieStore.get('obj'), function (data) {
        //data = JSON.parse(data);
        if (data.error) {
            ngDialog.open({
                template: '<p>Something went wrong !</p>',
        ...
        console.log(data);
        $scope.current.bookings = data.data.noOfBookings;
        $scope.current.customers = data.data.noOfCustomers;
        $scope.current.drivers = data.data.allSp;
